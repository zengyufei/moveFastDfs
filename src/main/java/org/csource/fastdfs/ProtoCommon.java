...
*/
	...
	/**
	 * split metadata to name value pair map 新加方法
	 * @param meta_buff metadata
	 * @return name value pair array
	 */
	public static Map<String, String> split_metadata_map(String meta_buff)
	{
		return split_metadata_map(meta_buff, FDFS_RECORD_SEPERATOR, FDFS_FIELD_SEPERATOR);
	}
...
*/
			...
			String  recordSeperator, String  filedSeperator)
	{
		...
		rows = meta_buff.split(recordSeperator);
		metaMap = new HashMap<String, String>(rows.length);
		for (int i=0; i<rows.length; i++)
		{
			cols = rows[i].split(filedSeperator, 2);
			if (cols.length == 2)
			{
				metaMap.put(cols[0], cols[1]);
			}
		}
		return metaMap;
	}
	...
	/**
	* pack metadata map to string 新加方法
	* @param meta_list metadata array
	* @return packed metadata
	*/
	public static String pack_metadata(Map<String, String> metaMap)
	{		
		if (metaMap.size() == 0)
		{
			return "";
		...
		StringBuffer sb = new StringBuffer(32 * metaMap.size());
		for (Entry<String, String> meta : metaMap.entrySet()) {
			sb.append(FDFS_RECORD_SEPERATOR);
			sb.append(meta.getKey()).append(FDFS_FIELD_SEPERATOR).append(meta.getValue());
		}
...
*/
 ...
 {
 	byte[] bsFilename = remote_filename.getBytes(Configuration.g_charset);
 	byte[] bsKey = secret_key.getBytes(Configuration.g_charset);
 	byte[] bsTimestamp = (new Integer(ts)).toString().getBytes(Configuration.g_charset);
