...
*/
	...
	*/
			...
			NameValuePair[] meta_list) throws IOException, MyException
	{
		final String group_name = null;
		//文件名后缀
		String file_ext_name = local_filename.substring(local_filename.lastIndexOf(".")+1, local_filename.length());
		return this.upload_file(group_name, local_filename, file_ext_name, meta_list);
	}
	...
	*/
	public String[] upload_file(String local_filename, String file_ext_name, 
			Map<String, String> metaMap) throws IOException, MyException
	{
		final String group_name = null;
		return this.upload_file(group_name, local_filename, file_ext_name, metaMap);
	}
	...
	*/
			...
			Map<String, String> metaMap) throws IOException, MyException
	{
		...
		return this.upload_file(cmd, group_name, local_filename, file_ext_name, metaMap);
	}
			...
			Map<String, String> metaMap) throws IOException, MyException
	{
		...
		try
		{
		  return this.do_upload_file(cmd, group_name, null, null, file_ext_name, 
		           f.length(), new UploadStream(fis, f.length()), metaMap);
		}
		finally
		{
			fis.close();
	...
	*/
	public String[] upload_file(String group_name, byte[] file_buff, int offset, int length,	
	       String file_ext_name, Map<String, String> metaMap) throws IOException, MyException
	{
		return this.do_upload_file(ProtoCommon.STORAGE_PROTO_CMD_UPLOAD_FILE, group_name, null, null, file_ext_name, 
		           length, new UploadBuff(file_buff, offset, length), metaMap);
	}
	/**
	 * upload file to storage server (by file buff)
	 * @param group_name the group name to upload file to, can be empty
	...
	*/
	...
	{
		final String group_name = null;
		return this.upload_file(group_name, file_buff, 0, file_buff.length, file_ext_name, metaMap);
	}
	/**
	 * upload file to storage server (by file buff)
	 * @param file_buff file content/buff
  ...
  * @param file_ext_name file ext name, do not include dot(.)
	...
	*         return null if fail
	*/
	protected String[] do_upload_file_map(byte cmd, String group_name, String master_filename, 
	         ...
	         Map<String, String> metaMap) throws IOException, MyException
	{
		...
		try
		{
			...
			{
				byte[] bs = file_ext_name.getBytes(Configuration.g_charset);
			...
			if (bUploadSlave)
			{
				masterFilenameBytes = master_filename.getBytes(Configuration.g_charset);
        ...
        body_len = sizeBytes.length + ProtoCommon.FDFS_FILE_EXT_NAME_MAX_LEN + file_size;
			...
			if (bUploadSlave)
			{
				...
				byte[] bs = prefix_name.getBytes(Configuration.g_charset);
			...
			try
			{
				result = this.set_metadata(new_group_name, remote_filename, 
						metaMap, ProtoCommon.STORAGE_SET_METADATA_FLAG_OVERWRITE);
			}
	...
	*/
	         ...
	         long file_size, UploadCallback callback) throws IOException, MyException
	{
		...
		try
		{
			...
			appenderFilenameBytes = appender_filename.getBytes(Configuration.g_charset);
  ...
  * @param file_offset the start offset of the file
	...
	*/
	public NameValuePair[] get_metadata(String group_name, String remote_filename)throws IOException, MyException
	{
		...
		try
		{
			...
			return ProtoCommon.split_metadata(new String(pkgInfo.body, Configuration.g_charset));
		}
		catch(IOException ex)
		{
			if (!bNewConnection)
	 ...
	 * @return meta info array, return null if fail
	 */
	public Map<String, String> get_metadata_map(String group_name, String remote_filename)throws IOException, MyException
	{
		boolean bNewConnection = this.newUpdatableStorageConnection(group_name, remote_filename);
		Socket storageSocket = this.storageServer.getSocket();
		...
		try
		{
			...
			return ProtoCommon.split_metadata_map(new String(pkgInfo.body, Configuration.g_charset));
		}
		catch(IOException ex)
		{
			if (!bNewConnection)
	...
	*/
	public int set_metadata(String group_name, String remote_filename, 
			Map<String, String> metaMap, byte op_flag) throws IOException, MyException
	{
		String meta_buff = ProtoCommon.pack_metadata(metaMap);
		return this.set_metadata(group_name, remote_filename, meta_buff, op_flag);
	}
	/**
	...
	{
		String meta_buff = ProtoCommon.pack_metadata(meta_list);
		return this.set_metadata(group_name, remote_filename, meta_buff, op_flag);
	}
	...
	*/
	public int set_metadata(String group_name, String remote_filename, 
							String meta_list, byte op_flag) throws IOException, MyException
	{
		boolean bNewConnection = this.newUpdatableStorageConnection(group_name, remote_filename);
		Socket storageSocket = this.storageServer.getSocket();
		...
		try
		{
			...
			{
				meta_buff = meta_list.getBytes(Configuration.g_charset);
			}
			
			filenameBytes = remote_filename.getBytes(Configuration.g_charset);
			sizeBytes = new byte[2 * ProtoCommon.FDFS_PROTO_PKG_LEN_SIZE];
			...
			bs = group_name.getBytes(Configuration.g_charset);
